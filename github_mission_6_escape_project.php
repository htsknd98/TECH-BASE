...
try{ //PDO設定
	...
	$pname = $_GET[pname];
 	...
 	if($_GET[yes]==1){
 		...
 		$sql="delete from project6 where pid=$pname and gid = $gname and mem='$myname'";
 		$stmt=$pdo->query($sql);
 		
 		$sql="delete from mission2 where pid=$pname and gid = $gname and mem='$myname'";
 		$stmt=$pdo->query($sql);
		...
		$stmt=$pdo->query($sql);
	 	...
	 	$user = '連絡';
	 	$comment = $myname.'が'.'プロジェクト:'.$project.'から脱退しました。';
	...
	<div class='all'>
		<?php
			if($login==0){
				...
				if($_GET[yes]==1){
					echo $project."から脱退しました。<br>";
				}else{
					echo $project."から脱退しますか？<br>";
					echo "<a href = 'github_mission_6_escape_project.php?gname=";	
					echo $gname;
					echo "&pname=";
					echo $pname;
					echo "&id=";
